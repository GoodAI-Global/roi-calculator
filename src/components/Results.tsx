  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center">
        <span className="w-8 h-8 bg-goodai-blue text-white rounded-full flex items-center justify-center text-sm mr-3">
          2
        </span>
        ROI Results
      </h2>
      ...
      <div className="mt-4 bg-gradient-to-r from-slate-50 to-gray-50 rounded-lg p-4 border border-slate-200">
        <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center">
          <svg
            className="w-4 h-4 mr-2 text-goodai-blue"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <div className="bg-white rounded p-3 border border-slate-100">
            ...
            <p
              className={`text-lg font-bold ${result.financialMetrics.npv >= 0 ? 'text-green-600' : 'text-red-600'}`}
            >
              {formatCurrency(result.financialMetrics.npv)}
            </p>
            <p className="text-xs text-gray-400">
              at {formatPercentage(result.financialConfig.discountRate * 100)} discount
            </p>
          </div>
          <div className="bg-white rounded p-3 border border-slate-100">
            <p className="text-xs text-gray-500 uppercase tracking-wide">IRR</p>
            <p
              className={`text-lg font-bold ${result.financialMetrics.irr > result.financialConfig.discountRate ? 'text-green-600' : 'text-amber-600'}`}
            >
          ...
          <div className="bg-white rounded p-3 border border-slate-100">
            ...
            <p className="text-xs text-gray-400">
              {result.financialConfig.analysisYears}-year total cost
            </p>
          </div>
          <div className="bg-white rounded p-3 border border-slate-100">
            <p className="text-xs text-gray-500 uppercase tracking-wide">Profitability Index</p>
            <p
              className={`text-lg font-bold ${result.financialMetrics.profitabilityIndex >= 1 ? 'text-green-600' : 'text-amber-600'}`}
            >
              {result.financialMetrics.profitabilityIndex.toFixed(2)}x
            </p>
            <p className="text-xs text-gray-400">
              {result.financialMetrics.profitabilityIndex >= 1
                ? 'Value creating'
                : 'Value destroying'}
            </p>
          </div>
        ...
        {result.financialMetrics.discountedPaybackMonths !== Infinity && (
          <p className="mt-3 text-xs text-gray-500">
            Discounted payback: {result.financialMetrics.discountedPaybackMonths.toFixed(1)} months
      ...
      {(isNegativeROI || isLongPayback) && (
        <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <p className="text-sm text-amber-800">
            <strong>Note:</strong> These projections show extended payback. Consider adjusting
            inputs or evaluating non-financial benefits.
          </p>
        </div>
      )}
    </div>
